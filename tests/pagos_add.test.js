...
beforeAll(async () => {
  await request(app).post('/api/auth/register').send({ nombre: 'PagoAddTester', email: 'pagos.add@example.com', password: 'testpass', rol: 'admin' });
  const loginRes = await request(app).post('/api/auth/login').send({ email: 'pagos.add@example.com', password: 'testpass' });
...
describe('POST /api/pedidos-venta/:id/pagos', () => {
  test('Registra pago adicional y aplica tasa según banco', async () => {
    // crear forma
    const forma = await request(app).post('/api/formas-pago').set(authHeader).send({ nombre: 'PagoAdd' });
    ...
    const banco = await request(app).post('/api/bancos').set(authHeader).send({ nombre: 'Banco PagoAdd', formas_pago: [{ forma_pago_id: formaId, detalles: {} }], moneda: 'VES_ADD' });
    expect(banco.statusCode).toBe(201);
    const bancoId = banco.body.id;

    // crear tasa activa
    const tasa = await request(app).post('/api/tasas-cambio').set(authHeader).send({ monto: 8.5, simbolo: 'VES', descripcion: 'tasa add', activo: true });
    ...
    // crear pedido (public)
    const pedidoBody = { cliente_id: 1, productos: [{ producto_id: prod.id, cantidad: 1 }], estado: 'Pendiente', nombre_cliente: 'Cliente PagoAdd' };
    ...
    const pagoPayload = { forma_pago_id: formaId, banco_id: bancoId, monto: 10.00, referencia: 'ADD-PAGO-1', fecha_transaccion: new Date().toISOString() };
    const pagoRes = await request(app).post(`/api/pedidos-venta/${pedido.id}/pagos`).set(authHeader).send(pagoPayload);
    ...
    const found = (pagosList.body || []).find(p => Number(p.pedido_venta_id) === pedido.id && p.referencia === 'ADD-PAGO-1');
