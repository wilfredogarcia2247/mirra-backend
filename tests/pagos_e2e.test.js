...
beforeAll(async () => {
  ...
  const loginRes = await request(app)
    .post('/api/auth/login')
    .send({ email: 'pago.tester@example.com', password: 'testpass' });
...
describe('Pagos E2E: finalizar pedido con pago y tasa aplicada', () => {
  test('Crear pedido y finalizar con pago que registra tasa y simbolo', async () => {
    ...
    const invRes = await request(app)
      ...
      .set(authHeader)
      .send({ almacen_id: ventaAlmacen.id, cantidad: 10 });
    ...
    const pedidoBody = {
      ...
      estado: 'Pendiente',
      nombre_cliente: 'Cliente PagoTest',
    };
    ...
    const pagoPayload = {
      pago: {
        ...
        banco_id: bancoId,
        monto: 20.0,
        referencia: 'TEST-PAGO-1',
        fecha_transaccion: new Date().toISOString(),
      },
    };
    const finalizarRes = await request(app)
      .post(`/api/pedidos-venta/${pedido.id}/finalizar`)
      .set(authHeader)
      .send(pagoPayload);
    ...
    const pagosForPedido = (pagosList.body || []).filter(
      (p) => Number(p.pedido_venta_id) === pedido.id
    );
