...
beforeAll(async () => {
  ...
  await request(app).post('/api/auth/register').send({
    nombre: 'TPtester',
    email: 'tp.tester@example.com',
    password: 'testpass',
    rol: 'admin',
  });
  const loginRes = await request(app)
    .post('/api/auth/login')
    .send({ email: 'tp.tester@example.com', password: 'testpass' });
...
describe('Pedido precio snapshot', () => {
  test('Al crear pedido se guarda precio_venta y no cambia al actualizar producto', async () => {
    ...
    const updRes = await request(app).put(`/api/productos/${prod.id}`).set(authHeader).send({
      nombre: prod.nombre,
      unidad: prod.unidad,
      stock: prod.stock,
      costo: prod.costo,
