...
beforeAll(async () => {
  await request(app).post('/api/auth/register').send({ nombre: 'PagoQueryTester', email: 'pagos.query@example.com', password: 'testpass', rol: 'admin' });
  const loginRes = await request(app).post('/api/auth/login').send({ email: 'pagos.query@example.com', password: 'testpass' });
...
describe('GET /api/pedidos-venta/:id/pagos', () => {
  test('Devuelve pagos asociados a un pedido', async () => {
    // crear forma
    const forma = await request(app).post('/api/formas-pago').set(authHeader).send({ nombre: 'QueryPago' });
    expect(forma.statusCode).toBe(201);
    const formaId = forma.body.id;

    // crear banco
    const banco = await request(app).post('/api/bancos').set(authHeader).send({ nombre: 'Banco QueryPago', formas_pago: [{ forma_pago_id: formaId, detalles: {} }], moneda: 'QRY' });
    ...
    const pedidoBody = { cliente_id: 1, productos: [{ producto_id: prod.id, cantidad: 1 }], estado: 'Pendiente', nombre_cliente: 'Cliente Query' };
    ...
    const pagoPayload = { forma_pago_id: formaId, banco_id: bancoId, monto: 10.00, referencia: 'QRY-PAGO-1', fecha_transaccion: new Date().toISOString() };
    ...
    expect(Number(found.monto)).toBeCloseTo(10.00, 5);
  });
});
