...
async function main() {
  ...
  if (!force) {
    if (dryRun) {
      console.log(
        'Modo dry-run activado: no se realizará ningún borrado. Mostrando información de lo que se eliminaría:'
      );
    } else {
    ...
    if (dryRun) {
      try {
        ...
        const inventarioRefs =
          await sql`SELECT COUNT(*)::int AS c FROM inventario WHERE producto_id IS NOT NULL`;
        ...
        console.log(
          'NOTA: El TRUNCATE propuesto sería: TRUNCATE TABLE productos RESTART IDENTITY CASCADE;'
        );
      ...
      try {
        await sql`DELETE FROM inventario WHERE producto_id IS NULL;`;
      } catch (e) {}
      await sql`COMMIT`;
