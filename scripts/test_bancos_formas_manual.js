...
(async function main() {
  try {
    ...
    await request(app)
      ...
      .send({ nombre: 'Tester Manual', email, password: 'testpass', rol: 'admin' });
    // Login
    ...
    console.log(
      ...
      formasRes.body.map((f) => ({ id: f.id, nombre: f.nombre }))
    );

    const transferencia = formasRes.body.find((f) => f.nombre === 'Transferencia');
    const pagoMovil = formasRes.body.find(
      (f) => f.nombre === 'Pago Movil' || f.nombre === 'Pago Móvil'
    );
