...
async function main() {
  ...
  try {
    ...
    const rows = await sql`SELECT p.*, b.nombre as banco_nombre, b.moneda as banco_moneda, f.nombre as forma_nombre FROM pagos p LEFT JOIN bancos b ON b.id = p.banco_id LEFT JOIN formas_pago f ON f.id = p.forma_pago_id WHERE p.pedido_venta_id = ${pedidoId} ORDER BY fecha DESC`;
    if (!rows || rows.length === 0) {
      console.log('No se encontraron pagos para el pedido', pedidoId);
      // Mostrar conteo rápido de filas en la tabla pagos
      try {
        const cnt = await sql`SELECT COUNT(*)::int AS c FROM pagos WHERE pedido_venta_id = ${pedidoId}`;
    ...
    for (const r of rows) {
      console.log('---');
      console.log('id:', r.id, 'monto:', r.monto, 'forma:', r.forma_nombre || r.forma_pago_id, 'banco:', r.banco_nombre || r.banco_id);
