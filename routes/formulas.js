...
function validarFormula(body) {
  ...
  if (body.costo == null || isNaN(Number(body.costo)))
    return 'Costo de la fórmula requerido y debe ser numérico';
  if (body.precio_venta == null || isNaN(Number(body.precio_venta)))
    return 'Precio de venta de la fórmula requerido y debe ser numérico';
  if (!Array.isArray(body.componentes) || body.componentes.length === 0)
    return 'Componentes requeridos';
  for (const c of body.componentes) {
    if (!c.materia_prima_id || isNaN(Number(c.materia_prima_id)))
      return 'ID de materia prima requerido';
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    formula[0].componentes =
      await sql`SELECT * FROM formula_componentes WHERE formula_id = ${formula[0].id}`;
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    updated[0].tamano = {
      ...
      cantidad: updated[0].cantidad || null,
      unidad: updated[0].unidad || null,
      costo: updated[0].costo,
      precio_venta: updated[0].precio_venta,
    };
    res.json(updated[0]);
  } catch (err) {
    try {
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    const ops =
      await sql`SELECT COUNT(*)::int AS c FROM ordenes_produccion WHERE formula_id = ${formulaId}`;
    if (ops && ops[0] && Number(ops[0].c) > 0)
...
router.post('/:id/produccion', async (req, res) => {
  ...
  if (cantidad == null || isNaN(Number(cantidad)) || Number(cantidad) <= 0)
    return res.status(400).json({ error: 'cantidad requerida y debe ser > 0' });
