...
router.post('/ajustar', async (req, res) => {
  const body = req.body || {};
  if (!body.producto_id || isNaN(Number(body.producto_id))) return res.status(400).json({ error: 'producto_id requerido' });
  if (!body.almacen_id || isNaN(Number(body.almacen_id))) return res.status(400).json({ error: 'almacen_id requerido' });
  if (body.cantidad == null || isNaN(Number(body.cantidad))) return res.status(400).json({ error: 'cantidad requerida y numérica' });
  const producto_id = Number(body.producto_id);
  const almacen_id = Number(body.almacen_id);
  const cantidad = Number(body.cantidad);
  try {
    // Buscar registro existente
    const rows = await sql`SELECT * FROM inventario WHERE producto_id = ${producto_id} AND almacen_id = ${almacen_id}`;
    if (rows && rows.length > 0) {
      ...
      if (nuevo < 0) return res.status(400).json({ error: 'No se puede reducir stock por debajo de 0' });
      if (nuevo < stockComprometido) return res.status(400).json({ error: 'No se puede reducir stock por debajo del stock comprometido' });
      const result = await sql`UPDATE inventario SET stock_fisico = ${nuevo} WHERE id = ${inv.id} RETURNING *`;
      return res.json(result[0]);
    } else {
      // No existe registro: solo permitir creación si cantidad >= 0
      if (cantidad < 0) return res.status(400).json({ error: 'No existe inventario para reducir en este almacén' });
      const result = await sql`INSERT INTO inventario (producto_id, almacen_id, stock_fisico, stock_comprometido) VALUES (${producto_id}, ${almacen_id}, ${cantidad}, 0) RETURNING *`;
